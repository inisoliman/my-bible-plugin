jQuery(document).ready(function($) {
    ...
    $(document.body).on('click', '.bible-control-button', function(event) {
        ...
        switch (action) {
            ...
            case 'toggle-commentary-menu':
                event.stopPropagation();
                const $dropdown = $button.siblings('.commentary-dropdown-menu');
                const $allDropdowns = $('.commentary-dropdown-menu');
    ...
    // Commentary dropdown functionality - FIXED
    $(document).on('click', '.commentary-toggle-button', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        const $dropdown = $(this).siblings('.commentary-dropdown-menu');
        $dropdown.toggleClass('active');
    });
    
    $(document).on('click', '.commentary-menu-item', function(e) {
        e.preventDefault();
        const selectedSource = $(this).data('source');
        const currentBook = $('#bible-book-select').val();
        const currentChapter = $('#bible-chapter-select').val();
        
        // Close dropdown
        $(this).closest('.commentary-dropdown-menu').removeClass('active');
        
        if (!currentBook || !currentChapter) {
            alert('يرجى اختيار السفر والأصحاح أولاً');
            return;
        }
        
        // Convert book name from Bible format to Commentary format via AJAX
        $.post(bibleFrontend.ajax_url, {
            action: 'convert_bible_to_commentary_book_name',
            nonce: bibleFrontend.nonce,
            book: currentBook
        }, function(response) {
            if (response.success && response.data.commentary_book_name) {
                const commentaryBookName = response.data.commentary_book_name;
                const bookSlug = commentaryBookName.replace(/\s+/g, '-');
                const commentaryUrl = `/bible-commentary/${selectedSource}/${encodeURIComponent(bookSlug)}/${currentChapter}/`;
                window.location.href = commentaryUrl;
            } else {
                // Fallback to direct conversion if AJAX fails
                const bookSlug = currentBook.replace(/\s+/g, '-');
                const commentaryUrl = `/bible-commentary/${selectedSource}/${encodeURIComponent(bookSlug)}/${currentChapter}/`;
                window.location.href = commentaryUrl;
            }
        });
    ...
    $(document).on('click', function(e) {
        if (!$(e.target).closest('.bible-commentary-dropdown-wrapper').length) {
            $('.commentary-dropdown-menu').removeClass('active');
        }
    });
